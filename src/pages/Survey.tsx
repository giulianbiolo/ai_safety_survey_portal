...
export function Survey() {
  ...
  const { completeSurvey, surveyCompleted, userId } = useAppStore();
  const navigate = useNavigate();

  useEffect(() => {
    if (surveyCompleted) {
      navigate("/scenario/1", { replace: true });
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      await submitSurvey(userId!, answers);
      ...
      <div className="max-w-3xl w-full mx-auto p-6 flex-1 flex flex-col">
        <div className="mb-8">
          ...
          <p className="text-zinc-400 mt-2">
            Please answer all questions before proceeding to the coding
            scenarios.
          </p>
        </div>
